...
adminRouter.get("/", (req, res) => {
  const db = getDb();
  const products = db
    .prepare("SELECT * FROM insurance_products ORDER BY sort_order ASC")
    .all();
...
adminRouter.post("/", validateCsrf, (req, res) => {
  ...
  const maxOrder =
    db.prepare("SELECT MAX(sort_order) as m FROM insurance_products").get().m ||
    0;
  db.prepare(
    `
  ...
  `,
    ...
    parseInt(sort_order) || maxOrder + 1,
    active === "on" ? 1 : 0,
  );
...
adminRouter.post("/:id/delete", validateCsrf, (req, res) => {
  getDb()
    .prepare("DELETE FROM insurance_products WHERE id = ?")
    .run(req.params.id);
...
adminRouter.post("/:id/toggle", validateCsrf, (req, res) => {
  const db = getDb();
  const product = db
    .prepare("SELECT active FROM insurance_products WHERE id = ?")
    .get(req.params.id);
  if (!product) return res.status(404).json({ error: "Not found" });
  const newVal = product.active ? 0 : 1;
  db.prepare("UPDATE insurance_products SET active = ? WHERE id = ?").run(
    newVal,
    req.params.id,
  );
...
adminRouter.post("/:id/move", validateCsrf, (req, res) => {
  ...
  const a = products[idx],
    b = products[swapIdx];
  db.prepare("UPDATE insurance_products SET sort_order = ? WHERE id = ?").run(
    b.sort_order,
    a.id,
  );
  db.prepare("UPDATE insurance_products SET sort_order = ? WHERE id = ?").run(
